...
### 핵심 개념
- **집합 연산자(Set Operator)**: 두 개 이상의 SELECT 결과(릴레이션)를 하나로 결합하는 연산자. 양쪽 SELECT의 **컬럼 수·데이터 타입·순서가 일치**해야 함.
- **UNION**(합집합, 중복 제거), **UNION ALL**(합집합, 중복 허용), **INTERSECT**(교집합), **MINUS/EXCEPT**(차집합).
- 오라클은 `MINUS`, 표준 SQL·SQL Server는 `EXCEPT`를 사용함.
...
### 핵심 개념
- **서브쿼리(Subquery)**: 다른 SQL 문의 내부에 **중첩된 SELECT**. 괄호로 묶어 사용하며, 메인쿼리보다 **먼저 실행**되어 결과를 전달.
- 종류: 단일행 서브쿼리(결과 1행), 다중행 서브쿼리(`IN`·`ANY`·`ALL`·`EXISTS` 사용), 다중컬럼 서브쿼리.
...
### 핵심 개념
- **인덱스(Index)**: 데이터베이스에 저장된 자료를 **빠르게 조회**하기 위해 사용하는 보조 자료구조. DDL로 생성(`CREATE INDEX`)·변경·제거 가능.
- **특징**: 테이블의 **논리적 구조와 밀접**하게 연관됨. 검색 성능은 향상되지만 삽입·삭제·갱신 시 인덱스 갱신 비용이 발생.
- 레코드 삽입·삭제가 수시인 경우 인덱스 개수를 **최소화**하여 갱신 비용 증가를 방지.
