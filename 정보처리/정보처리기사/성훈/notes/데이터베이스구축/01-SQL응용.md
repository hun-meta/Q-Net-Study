...
### 핵심 개념
- **그룹 함수(집계 함수)**: 여러 행을 그룹화하여 하나의 결과값을 반환. `COUNT`(행 수), `SUM`(합계), `AVG`(평균), `MAX`(최댓값), `MIN`(최솟값).
- `GROUP BY 컬럼`: 지정 컬럼 값이 같은 행끼리 하나의 그룹으로 묶음. SELECT 절에는 **그룹 기준 컬럼과 집계 함수만** 올 수 있음.
- `HAVING 조건`: 그룹에 대한 조건 필터. **반드시 GROUP BY 절과 함께 사용**하며 그룹화 **이후**에 적용.
- **WHERE vs HAVING**: WHERE는 그룹화 **전** 개별 행 조건, HAVING은 그룹화 **후** 그룹 단위 조건.
...
### 핵심 개념
- SQL 논리 연산자: `AND`(모두 참), `OR`(하나라도 참), `NOT`(부정). (**OTHER는 존재하지 않는** 연산자)
- `BETWEEN A AND B`: A 이상 B 이하(양끝 포함). 반드시 **AND**로 연결하며 `BETWEEN ... OR`는 **문법 오류**.
- `IN(값1, 값2)`: OR의 축약형. `LIKE '패턴'`: `%`(0개 이상 임의 문자), `_`(1개 임의 문자) 와일드카드 사용.
- 널 값 비교는 `=`·`<>`가 아닌 `IS NULL` / `IS NOT NULL` 사용.
