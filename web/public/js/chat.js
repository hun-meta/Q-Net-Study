// 문항 챗 UI: agy 스트리밍 답변(표시만) + 승인 정리(claude 직접 쓰기, 목적지 선택).
//
  ...
  async function 질문전송() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        답.textContent = data.error || '챗 요청 실패';
        답.classList.add('error');
        return;
      }
      let acc = '';
      await 스트림읽기(res, (evt) => {
