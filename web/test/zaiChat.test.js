...
test('streamChat: 기본 effort(none) → 요청 바디에 thinking disabled', async () => {
  ...
  // usage(캐시 히트 관측) 수신을 위한 OpenAI 호환 옵션은 항상 켠다.
...
test('streamChat: meta 주입 → system이 역할→문항 메타→문항 컨텍스트 순서(접두사 고정)', async () => {
  let capturedBody = null;
  await zaiChat.streamChat({
    zai: BASE_ZAI,
    meta: { grade: '정보처리', cert: '정보처리기사', examId: '2023-1-필기', qno: '5' },
    contextText: '[문항 원문] 1번 문제',
    message: 'q',
    fetchImpl: async (url, opts) => {
      capturedBody = JSON.parse(opts.body);
  ...
  assert.ok(sys.content.includes('자격증: 정보처리 / 정보처리기사'));
  assert.ok(sys.content.includes('시험: 2023-1-필기'));
  assert.ok(sys.content.includes('문번: 5'));
  const iRole = sys.content.indexOf('# 역할');
  const iMeta = sys.content.indexOf('# 문항 메타');
  const iCtx = sys.content.indexOf('# 문항 컨텍스트');
  assert.strictEqual(iRole, 0);
  assert.ok(iRole < iMeta && iMeta < iCtx, `순서 오류: 역할=${iRole} 메타=${iMeta} 컨텍스트=${iCtx}`);
});

test('streamChat: meta 없으면 문항 메타 블록 자체가 생략(기존 프롬프트와 동일)', async () => {
  let capturedBody = null;
  await zaiChat.streamChat({
    zai: BASE_ZAI,
    message: 'q',
...
test('streamChat: usage 청크 수신 → res.usage에 cached_tokens 포함 그대로 보존', async () => {
  const chunks = [
    'data: {"choices":[{"delta":{"content":"답"}}]}\n\n',
    // stream_options.include_usage 응답 형태: 마지막에 choices 빈 배열 + usage 청크.
    'data: {"choices":[],"usage":{"prompt_tokens":1200,"completion_tokens":40,"prompt_tokens_details":{"cached_tokens":1100}}}\n\n',
    'data: [DONE]\n\n',
  ];
  const res = await zaiChat.streamChat({
    zai: BASE_ZAI,
  ...
  assert.strictEqual(res.text, '답');
  assert.strictEqual(res.usage.prompt_tokens, 1200);
  assert.strictEqual(res.usage.completion_tokens, 40);
  assert.strictEqual(res.usage.prompt_tokens_details.cached_tokens, 1100);
});

test('streamChat: usage 청크가 없으면 res.usage는 null', async () => {
  const res = await zaiChat.streamChat({
    zai: BASE_ZAI,
    message: 'q',
    fetchImpl: async () => makeSseResponse(['data: [DONE]\n\n']),
  });
