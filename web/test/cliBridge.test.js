...
test('buildChatMeta: 자격증·시험·문번 블록 조립, 값 없으면 빈 문자열', () => {
  const m = bridgeMod.buildChatMeta({
    grade: '정보처리',
    cert: '정보처리기사',
    examId: '2023-1-필기',
    qno: '5',
  });
  assert.ok(m.startsWith('# 문항 메타'));
  assert.ok(m.includes('자격증: 정보처리 / 정보처리기사'));
  assert.ok(m.includes('시험: 2023-1-필기'));
  assert.ok(m.includes('문번: 5'));
  assert.strictEqual(bridgeMod.buildChatMeta({}), '');
  assert.strictEqual(bridgeMod.buildChatMeta(), '');
  // 같은 입력 → 바이트 동일 출력(접두사 캐싱 히트 조건).
  assert.strictEqual(
    m,
    bridgeMod.buildChatMeta({ grade: '정보처리', cert: '정보처리기사', examId: '2023-1-필기', qno: '5' })
  );
});

test('buildChatPrompt: 문항 메타가 역할 뒤·문항 컨텍스트 앞에 삽입(agy 경로 동등성)', () => {
  const p = bridgeMod.buildChatPrompt({
    grade: '정보처리',
    cert: '정보처리기사',
    examId: '2023-1-필기',
    qno: '5',
    contextText: '[문항 원문] 본문',
    message: 'q',
  });
  const iRole = p.indexOf('# 역할');
  const iMeta = p.indexOf('# 문항 메타');
  const iCtx = p.indexOf('# 문항 컨텍스트');
  assert.strictEqual(iRole, 0);
  assert.ok(iMeta > iRole && iCtx > iMeta, `순서 오류: 역할=${iRole} 메타=${iMeta} 컨텍스트=${iCtx}`);
  // 메타 없는 기존 호출은 블록 자체가 생략(회귀).
  assert.ok(!bridgeMod.buildChatPrompt({ message: 'q' }).includes('# 문항 메타'));
});
...
// agy 경로 테스트는 resolveZai 비활성 스텁을 반드시 주입한다 — 개발 머신
// secrets.json 에 실 키가 등록돼 있으면 zai 경로로 새어 실제 API 를 호출하게 된다.
const ZAI_OFF = () => ({ enabled: false, apiKey: '', source: null, baseUrl: '', model: '', effort: '' });
...
test('createBridge.chat: Z.AI 활성(resolveZai 목 주입) 시 CLI 미감지여도 spawn 없이 zaiChat 경로로 응답', async () => {
  ...
  const job = await bridge.chat({
    ...
    fetchImpl: async (url, opts) => {
      ...
      return makeSseResponse([
        ...
        'data: {"choices":[],"usage":{"prompt_tokens":300,"completion_tokens":10,"prompt_tokens_details":{"cached_tokens":250}}}\n\n',
        'data: [DONE]\n\n',
      ]);
    },
  });
  ...
  // 라우트가 넘긴 검증 메타가 system 프롬프트에 주입된다(클라 contextText 무관).
  assert.ok(capturedBody.messages[0].content.includes('# 문항 메타'));
  assert.ok(capturedBody.messages[0].content.includes('시험: 2023-1-필기'));
  // usage(캐시 관측)가 브리지 결과로 그대로 전달된다.
  assert.strictEqual(job.usage.prompt_tokens_details.cached_tokens, 250);
});
