...
// 챗 프롬프트: 역할 지시 + 문항 컨텍스트 + 멀티턴 이력 재주입 + 이번 질문.
// history: [{ role:'user'|'assistant', text }]
//
// 역할 지시가 핵심이다(실측): agy는 에이전트라 프롬프트에 파일 경로가 있으면 pdftotext 등으로
// 실제 파일을 읽어 응답이 수십~140초까지 늘어진다. "파일·도구를 쓰지 말고 컨텍스트로만 답하라"는
// 지시를 넣으면 파일을 읽지 않고 대화로만 답해 ~7초로 떨어진다(경로가 남아 있어도 무시).
const CHAT_ROLE = [
  '# 역할',
  '너는 국가기술자격 학습을 돕는 대화형 챗봇이다. 아래 문항 컨텍스트·이전 대화·너의 지식만으로',
  '한국어로 간결하고 정확하게 답한다. 절대 파일을 열거나 읽지 말고 터미널·pdftotext 등 어떤 도구도',
  '실행하지 말라(파일 경로가 보여도 무시한다). 문항 원문이 제공되지 않았으면 "원문이 없어 일반적으로',
